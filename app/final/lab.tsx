...
function Chemistry3D({
      ...
      PanResponder.create({
        ...
        onPanResponderMove: (_, g) => {
          if (g.numberActiveTouches === 1) {
            ...
            cameraSpherical.current.phi = Math.min(
              Math.max(0.1, cameraSpherical.current.phi),
              Math.PI - 0.1
            );
            ...
            if (touches?.length >= 2) {
              ...
              if (lastPinchDistance.current != null) {
                ...
                cameraSpherical.current.radius = Math.min(
                  Math.max(1.5, cameraSpherical.current.radius),
                  10
                );
              }
  ...
  useEffect(() => {
    ...
    async function onContextCreate(gl: any) {
      try {
        ...
        renderer = new THREE.WebGLRenderer({ canvas: gl.canvas as any, context: gl, antialias: true });
        ...
        // Prevent uncaught errors in release that would crash the app
        // console.warn("Chemistry3D init failed:", err);
      }
    }
  ...
  return (
    <View {...panResponder.panHandlers} style={{ flex: 1 }}>
      <GLView style={{ flex: 1 }} onContextCreate={(gl) => { glRef.current = gl; }} />
    </View>
  );
}
...
function PhysicsSimulation({ gravityStrength }: { gravityStrength: number }) {
  ...
  useEffect(() => {
    ...
    return () => {
      mounted = false;
      if (requestIdRef.current != null) cancelAnimationFrame(requestIdRef.current);
      particlesRef.current.forEach((m) => {
        try {
          m.geometry?.dispose();
          (m.material as any)?.dispose?.();
        } catch {}
